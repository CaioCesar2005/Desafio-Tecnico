...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<StandardError> handleClienteNotFound(ClienteNotFoundException ex, HttpServletRequest request) {
        StandardError erroPadrao = montarErroPadrao(HttpStatus.NOT_FOUND, "Cliente não encontrado", ex.getMessage(), request.getRequestURI());
    ...
    public ResponseEntity<StandardError> handleContaNotFound(ContaNotFoundException ex, HttpServletRequest request) {
        StandardError erroPadrao = montarErroPadrao(HttpStatus.NOT_FOUND, "Conta não encontrada", ex.getMessage(), request.getRequestURI());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(erroPadrao);
    }

    @ExceptionHandler(RegraNegocioException.class)
    public ResponseEntity<StandardError> handleRegraNegocio(RegraNegocioException ex, HttpServletRequest request) {
        StandardError erroPadrao = montarErroPadrao(HttpStatus.BAD_REQUEST, "Violação de regra de negócio", ex.getMessage(), request.getRequestURI());
    ...
    public ResponseEntity<ValidationError> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        ...
        ex.getBindingResult().getFieldErrors().forEach(e -> {
            erroValidacao.adicionarErroCampo(e.getField(), e.getDefaultMessage());
        });
    ...
    public ResponseEntity<StandardError> handlePathParamError(MethodArgumentTypeMismatchException ex, HttpServletRequest request) {
        ...
        StandardError erroPadrao = montarErroPadrao(HttpStatus.BAD_REQUEST, "Parâmetro inválido", mensagem, request.getRequestURI());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(erroPadrao);
    }
    ...
    public ResponseEntity<StandardError> handleException(Exception ex, HttpServletRequest request) {
        ...
        StandardError erroPadrao = montarErroPadrao(HttpStatus.INTERNAL_SERVER_ERROR, "Erro inesperado", "Ocorreu um erro interno no servidor", request.getRequestURI());
